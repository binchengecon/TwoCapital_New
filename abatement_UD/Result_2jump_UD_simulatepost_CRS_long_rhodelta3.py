...
def simulate_pre(
    ...
    delta, mu_k, kappa, sigma_k, beta_f, zeta, psi_0, psi_1, sigma_g, theta, lambda_bar, vartheta_bar, varrho, xi_a,xi_k,xi_c,xi_j,xi_d,xi_g,rho, gamma_3_i = model_args
    # ii, ee, xx, g_tech, g_damage, pi_c, h, h_k, h_j, v, v_post_tech_raw = controls
    ii, ee, xx, g_tech, pi_c, h, h_k, h_j, v = controls
    ME_base = ME
    n_bar = n_bar
    K_0, Y_0, L_0 = initial
...
def model_simulation_generate(xi_a,xi_k,xi_c,xi_j,xi_d,xi_g,rho,psi_0,psi_1,varrho):
    ...
    h =  model_tech1_post_damage_ME_base["h"]
    h_k =  model_tech1_post_damage_ME_base["h_k"]
    h_j =  model_tech1_post_damage_ME_base["h_j"]
    ...
    model_args = (delta, mu_k, kappa,sigma_k, beta_f, zeta, psi_0, psi_1, sigma_g, theta, lambda_bar, vartheta_bar, varrho, xi_a,xi_k,xi_c,xi_j,xi_d,xi_g, rho, gamma_3_i)



    res = simulate_pre(grid = (K, Y, L), 
                       model_args = model_args, 
                    #    controls = (i,e,x, g_tech, g_damage, pi_c, h, h_k, h_j, v, v_post_tech),
                       controls = (i,e,x, g_tech, pi_c, h, h_k, h_j, v),
                       ME = ME_family,
                       n_bar = 100,  
                       T0=0, 
                       T=IntPeriod, 
    ...
    with open(Data_Dir + File_Dir+"model_tech1_post_damage_gamma_{:.4f}".format(gamma_3_i)+"_UD_simul_{}".format(IntPeriod)+ scheme + "_" +HJB_solution, "wb") as f:
        pickle.dump(res,f)

    print(Data_Dir + File_Dir+"model_tech1_post_damage_gamma_{:.4f}".format(gamma_3_i)+"_UD_simul_{}".format(IntPeriod)+ scheme + "_" +HJB_solution)
