...
def _FOC_update(v0, steps= (), states = (), args=(), controls=(), fraction=0.5):
    ...
    if theta == 2 and psi_1 == 1:
        ...
        temp_mc =  delta * consumption**(-rho) * (np.exp(K_mat)/np.exp(v0))**(1-rho)
        
        delta_tilde = temp_mc * consumption
        
        
        
        temp_e_coef = delta_tilde/consumption * vartheta_bar * theta/ ( lambda_bar * np.exp(K_mat) )
        ...
        temp_i_coef = delta_tilde/consumption
        
        i_new = - (temp_i_coef / dK - 1) / kappa
        i_new[i_new <= 1e-16] = 1e-16
        
        temp_x_coef = delta_tilde/consumption
