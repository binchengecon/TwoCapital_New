...
def _FOC_update(v0, steps= (), states = (), args=(), controls=(), fraction=0.5):
    ...
    # D += xi_g * np.exp((L_mat - np.log(varrho))) * (1 - gg + gg * np.log(gg)) + np.exp( (L_mat - np.log(varrho)) ) * gg * (V_post_tech -  v0) 
...
def hjb_pre_tech(
    ...
    if v0 is None:
        # v0 = K_mat**2/2 + L_mat**2/2 - np.average(pi_c_o, axis=0) * Y_mat**2/2
    ...
    while FC_Err > tol and epoch < max_iter:
        ...
        if model == "Pre damage":
            ...
            D += xi_d * Intensity * np.sum( pi_d_o*(1-g_damage+g_damage*np.log(g_damage)),axis=0) +Intensity*np.sum(pi_d_o*g_damage*(v_i - v0),axis=0)
