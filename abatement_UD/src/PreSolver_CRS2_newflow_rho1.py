...
def _FOC_update(v0, steps= (), states = (), args=(), controls=(), fraction=0.5):
    ...
    delta, alpha, theta, vartheta_bar, lambda_bar, mu_k, kappa, sigma_k, theta_ell, pi_c_o, pi_c, sigma_y, zeta, psi_01, psi_02, psi_1, sigma_g, V_post_tech, dG, ddG, xi_a, xi_k, xi_c, xi_j, xi_d, xi_g, rho, varrho = args
    ...
    if theta == 2 and psi_1 == 1:
        ...
        x_new = (temp_ix_coef / (dL * psi_01 * psi_1) * np.exp(psi_1 * (L_mat - K_mat)) )**(1 / (psi_1 - 1))
    ...
    B_3 = - zeta + (psi_01+psi_02) * (xx * np.exp(K_mat - L_mat))**psi_1 - 0.5 * sigma_g**2
    B_3 += sigma_g*h_j
...
def hjb_pre_tech(
    ...
    delta, alpha, theta, vartheta_bar, lambda_bar, mu_k, kappa, sigma_k, theta_ell, pi_c_o, pi_c, sigma_y, zeta, psi_01, psi_02, psi_1, sigma_g, V_post_tech, gamma_1, gamma_2, gamma_3, y_bar, xi_a, xi_k, xi_c, xi_j, xi_d, xi_g, rho, varrho = model_args
    ...
    while FC_Err > tol and epoch < max_iter:
        ...
        FOC_args = (delta, alpha, theta, vartheta_bar, lambda_bar, mu_k, kappa, sigma_k, theta_ell, pi_c_o, pi_c, sigma_y, zeta, psi_01, psi_02, psi_1, sigma_g, V_post_tech, dG, ddG, xi_a, xi_k, xi_c, xi_j, xi_d, xi_g, rho, varrho)
        ...
        if FC_Err < 1.2*tol:
            ...
            if epoch%100==0:
                ...
                print("Epoch time: {:.4f}".format(time.time() - start_ep), flush =True)
        elif epoch%10000==0:
