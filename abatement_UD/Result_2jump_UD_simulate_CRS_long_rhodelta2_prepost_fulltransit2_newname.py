...
parser.add_argument("--TAJump", type=float)
...
def simulate_pre(
    ...
    for tm in range(pers):
        if tm == 0:
            ...
            if hist[tm,1]>=TAJump and hist[tm-1,1]<TAJump:
                
                gridpoints_long = (K, Y_long, L)
                
                i_post_func = RegularGridInterpolator(gridpoints_long, postdamage[id_damage]["i_star"])
                x_post_func = RegularGridInterpolator(gridpoints_long, postdamage[id_damage]["x_star"])
                e_post_func = RegularGridInterpolator(gridpoints_long, postdamage[id_damage]["e_star"])
                            
                        
                tech_post_func = RegularGridInterpolator(gridpoints_long, postdamage[id_damage]["g_tech"])
                
                # v_func = RegularGridInterpolator(gridpoints_long, v)
                
                # h_func = RegularGridInterpolator(gridpoints_long, h)
                # hk_func = RegularGridInterpolator(gridpoints_long, h_k)
                # hj_func = RegularGridInterpolator(gridpoints_long, h_j)
                # dL_func   = RegularGridInterpolator(gridpoints_long, dL)
                # dY_func   = RegularGridInterpolator(gridpoints_long, dY)
                # ME_total_func = RegularGridInterpolator(gridpoints_long, ME_total)
                # ME_base_func = RegularGridInterpolator(gridpoints_long, ME_base)
                ...
                # climate_func_list = []
                # for i in range(n_climate):
                #     func_i = RegularGridInterpolator(gridpoints_long, pi_c[i])
                #     climate_func_list.append(func_i)


                def get_i(x):
                    return i_post_func(x)

                def get_e(x):
                    return e_post_func(x)
                
                def get_x(x):
                    return x_post_func(x)
...
def model_simulation_generate(xi_a,xi_k,xi_c,xi_j,xi_d,xi_g,rho,psi_0,psi_1,varrho):
    ...
    with open(Data_Dir + File_Dir+"model_tech1_pre_damage_to_post_damage_{:.4f}".format(gamma_3_i)+"_UD_simul_fulltransit_{}_{}".format(IntPeriod,TAJump)+ scheme + "_" +HJB_solution, "wb") as f:
        pickle.dump(res,f)
