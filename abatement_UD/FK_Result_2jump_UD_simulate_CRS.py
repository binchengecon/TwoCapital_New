...
def simulate_pre(
    ...
    for tm in range(pers):
        if tm == 0:
            ...
            dL_hist[tm] = dL_func(hist[tm,:])
            dvdL_dis_hist[tm] = dvdL_dis_func(hist[tm,:])
            dvdL_undis_hist[tm] = dvdL_undis_func(hist[tm,:])
        ...
        if printing==True:
            ...
            print("time={}, xx={}, dL={}, dL_dis={}" .format(tm, x_hist[tm],dL_hist[tm], dvdL_dis_hist[tm]), flush=True)
    ...
    svrd_dis_hist = np.exp(hist[:,2]) * dvdL_dis_hist /(delta  * C**(-rho) * np.exp((rho-1)*vt))
    svrd_undis_hist = np.exp(hist[:,2]) * dvdL_undis_hist /(delta  * C**(-rho) * np.exp((rho-1)*vt))
...
def model_simulation_generate(xi_a,xi_k,xi_c,xi_j,xi_d,xi_g,rho,psi_0,psi_1,varrho):
    ...
    dvdL_dis = FK_Dis_tech1['dvdL']
    dvdL_undis = FK_Undis_tech1['dvdL']
    ...
    res = simulate_pre(grid = (K, Y_short, L), 
                       ...
                       ME = ME_family,
                       dV = (dvdL_dis, dvdL_undis),
                       n_bar = n_bar,  
                       T0=0, 
                       T=IntPeriod, 
                       dt=timespan,printing=True)

    with open(Data_Dir + File_Dir+"model_tech1_pre_damage"+"_FK_UD_simul_{}".format(IntPeriod)+ scheme + "_" +HJB_solution, "wb") as f:
        pickle.dump(res,f)
