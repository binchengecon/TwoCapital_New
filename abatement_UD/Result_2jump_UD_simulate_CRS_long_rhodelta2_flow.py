...
parser.add_argument("--psi01arr",nargs='+',type=float)
parser.add_argument("--psi02arr",nargs='+',type=float)
...
def simulate_pre(
    ...
    delta, mu_k, kappa, sigma_k, beta_f, zeta, psi_01, psi_02, psi_1, sigma_g, theta, lambda_bar, vartheta_bar, varrho, xi_a,xi_k,xi_c,xi_j,xi_d,xi_g,rho = model_args
    ...
    def mu_L(Xt, state):
        ...
        return -zeta + (psi_01 + psi_02) * Xt **psi_1 * (np.exp( psi_1 * (state[0]-state[2])) )  - 0.5 * sigma_g**2
    ...
    scrd_hist2 = (psi_01+psi_02) * psi_1 * (x_hist * np.exp(hist[:,0] - hist[:, 2]) ) ** psi_1
...
def model_simulation_generate(xi_a,xi_k,xi_c,xi_j,xi_d,xi_g,rho,psi_01,psi_02,psi_1,varrho):
    ...
    File_Dir = "xi_a_{}_xi_k_{}_xi_c_{}_xi_j_{}_xi_d_{}_xi_g_{}_psi_01_{}_psi_02_{}_psi_1_{}_varrho_{}_rho_{}_delta_{}_" .format(xi_a, xi_k, xi_c, xi_j, xi_d, xi_g, psi_01, psi_02, psi_1, varrho, rho, delta)
    ...
    model_args = (delta, mu_k, kappa,sigma_k, beta_f, zeta, psi_01, psi_02, psi_1, sigma_g, theta, lambda_bar, vartheta_bar, varrho, xi_a,xi_k,xi_c,xi_j,xi_d,xi_g, rho)
...
for id_xiag in range(len(xiaarr)): 
    for id_psi01 in range(len(psi01arr)):
        for id_psi02 in range(len(psi02arr)):
            for id_psi1 in range(len(psi1arr)):
                for id_varrho in range(len(varrhoarr)):
                    for id_rho in range(len(rhoarr)):

                        res = model_simulation_generate(xiaarr[id_xiag],xikarr[id_xiag],xicarr[id_xiag],xijarr[id_xiag],xidarr[id_xiag],xigarr[id_xiag],rhoarr[id_rho],psi01arr[id_psi01],psi02arr[id_psi02],psi1arr[id_psi1],varrhoarr[id_varrho])
