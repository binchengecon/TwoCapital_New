...
def simulate_pre(
    ...
    techjump_func   = RegularGridInterpolator(gridpoints, np.exp(L_mat)/varrho*g_tech)
    damagejump_func   = RegularGridInterpolator(gridpoints, Damage_Intensity(Y_mat)*np.sum(g_damage,axis=0)/len(g_damage))
    ...
    for tm in range(pers):
        if tm == 0:
            ...
            if tech_activate==0 and damage_activate ==1: 
                ...
                techjump_func   = RegularGridInterpolator(gridpoints, np.exp(L_matnew)/varrho*postdamage[id_3]["g_tech"])
...
def model_simulation_generate(xi_a,xi_c,xi_d,xi_g,psi_0,psi_1,varrho):
    ...
    for SimEpisodeNum in range(SimEpisode):
        ...
        with open(Data_Dir + File_Dir+"model_tech1_pre_damage"+"_FK_simuldistorted_{}_path_{}".format(IntPeriod, SimEpochNum*SimEpisode + SimEpisodeNum)+ scheme + "_" +HJB_solution, "wb") as f:
            pickle.dump(res,f)
