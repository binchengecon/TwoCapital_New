...
def dropbox_download_file(dropbox_file_path, local_file_path):
    ...
    try:
        dbx = dropbox.Dropbox(oauth2_access_token=access_token,
                              ...
                              app_key=APP_KEY,
                              app_secret=APP_SECRET)
        print("_________________List of File__________")
        print(dbx.files_list_folder(dropbox_file_path))
        for entry in dbx.files_list_folder(dropbox_file_path).entries:
            print(entry.name,flush=True)
        print("_______________________________________")

        # print("________________Downloading____________")

        # for entry in dbx.files_list_folder(dropbox_file_path).entries:
        #     entry_path = os.path.join(dropbox_file_path,entry.name)
        #     print(os.path.join(local_file_path,entry.name),flush=True)
        #     os.makedirs(local_file_path,exist_ok=True)
        #     with open(os.path.join(local_file_path,entry.name), 'wb') as f:
        #         metadata, result = dbx.files_download(path=entry_path)
        #         f.write(result.content)
        # print("_______________________________________")
...
dropbox_download_file(MotherFolder, DaugherFolder)
